      ...
      tabItem(tabName = "beranda",
              ...
              fluidRow(
                box(title = "Distribusi Akreditasi Universitas", 
                    plotlyOutput("akreditasi_univ_plot"), width = 6),  
                box(title = "Distribusi Akreditasi Program Studi", 
                    plotlyOutput("akreditasi_prodi_plot"), width = 6)  
              )
      ),
...
server <- function(input, output, session) {
  ...
  rm(data)  # Hapus variabel lama
  data <- read.csv("data/UNIVERSITAS.csv")  # Muat ulang dataset
  # Grafik Distribusi Kabupaten/Kota per Provinsi
  output$grafik_wilayah <- renderPlotly({
    data <- wilayah_data() %>%
      group_by(nama_prov) %>%
  ...
  # Ambil Data Universitas dari Database untuk Peta
  universitas_data <- reactive({
    data <- dbGetQuery(con, "SELECT nama_univ,latitude,longitude FROM Universitas")
    data <- na.omit(data) # Hapus baris yang memiliki nilai NA
    return(data)
  })
  
  # Peta Sebaran Universitas di Tab Wilayah
  output$peta_universitas <- renderLeaflet({
    data <- universitas_data()
    
    if(nrow(data) == 0) {
      return(NULL) # Jika data kosong, tidak menampilkan apa-apa
    }
    ...
    leaflet(data) %>%
      ...
      fitBounds(lng1 = min(data$longitude, na.rm = TRUE), 
